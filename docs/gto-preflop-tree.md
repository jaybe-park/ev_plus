...
별도 관리 TODO Task는 종료됨(2026-07-17, 상세: `TODO_ARCHIVE.md`) — 앞으로는
"퇴근 시 돌려놓기" 루틴에서 필요할 때 `scripts/collect_gto_tree.py`를 계속 돌리는
운영 작업으로 전환. 포스트플랍 추상화 계획(구 docs/gto-postflop.md, 폐기됨)과 대비 —
프리플랍은 근사가 아니라 완전 열거가 목표라 계속 유효.
...
### 세 번째 라이브 실행(`--limit 200`) 중 크롬 렌더러 크래시 + 노드 유실 방지 (2026-07-17)

**증상**: 사용자가 `--limit 200`으로 대량 실행 중 101번째 노드 근처에서 매 요청이
`Page.goto: Page crashed`로 실패하기 시작, 이후 20개(101~120)가 연속으로 똑같이
실패(자동 복구 없음). 사용자가 눈치채고 브라우저를 새로고침, 프로세스는 수동 중단.

**원인 조사(추측 아님 — 크롬 자체 크래시 리포터로 확인)**:
- `chrome://crashes`에 실제 크래시 기록이 남아있음(1건, 업로드 안 됨) → 이건 **크롬
  렌더러 프로세스 크래시**이지 서버 응답이 아니다. 서버가 우리를 막았다면 HTTP 에러나
  캡차/한도 배너(`X/100` 카운터)가 떴을 텐데 그런 신호는 전혀 없었음 — **서버 차단이
  아니라 브라우저 쪽 리소스 문제**로 결론.
- 유력 원인: 같은 탭에서 100번 넘게 반복 navigate → 누적 메모리/렌더링 부하(GTO Wizard
  자체의 JS 메모리 누수 가능성). 요청 속도(랜덤 지연)와는 별개 문제라, 지연을 늘려도
  근본 해결 안 됨 — **탭 자체를 주기적으로 재생성**하는 게 맞는 해법.

**부수 발견(사용자 지적) — 환경 오류가 영구 실패로 오염될 뻔함**: 크래시로 실패한
노드가 원래 코드에서는 `badSum` 같은 **진짜 데이터 이상**과 똑같이 취급돼 `failed`+
`visited`에 영구 기록되고 있었다(84개가 그렇게 됨 — 세션 중 수동으로 체크포인트를
DB 기준 재구성해 긴급 복구했음, 코드 수정 아닌 1회성 조치였음). 이번에 **코드 자체를
고쳐** 재발을 막음:

- `is_env_failure(reason)`: 실패 사유를 "환경 오류"(크래시/렌더타임아웃/JS추출실패/
  로컬서버 저장실패)와 "진짜 데이터 이상"(badSum, 핸드 0개)으로 구분하는 판정 함수 신설.
- **환경 오류는 `failed`/`visited`에 넣지 않고 그냥 프론티어에 되돌려 자동 재시도**한다
  (추측 채움이 아니라 "환경이 안정되면 같은 노드를 다시 시도"일 뿐이라 원칙에 위배 안 됨).
- **연속 2회 환경오류 → 탭 자동 재생성**(`recreate_page`: 죽은 탭 close + `ctx.new_page()`
  — 같은 브라우저 컨텍스트라 로그인 세션/쿠키는 그대로 유지됨).
- **연속 6회까지 갔는데도(재생성 이후 포함) 계속 실패 → 안전 중단**(서킷 브레이커) —
  의미 없이 나머지 `--limit`을 전부 태우며 좋은 프론티어 노드들을 잘못 실패 처리하는
  것을 막음. 체크포인트는 그대로 보존(재실행하면 이어감).
- **25개 처리마다 예방적 탭 재생성**(`PAGE_RECYCLE_EVERY=25`) — 크래시가 나기 전에
  선제적으로 탭을 새로 만들어 누적 메모리를 리셋. 크래시 자체의 재발 확률을 낮추는 목적.
- `save_node`(로컬 백엔드 저장) 실패도 같은 유실 버그가 있었음(팝된 노드를 안 돌려주고
  버림) — 동일하게 프론티어 복귀로 수정(백엔드 서버가 잠깐 죽은 것도 데이터 이상이
  아니라 환경 오류이므로).

**검증**: 몽키패치로 두 시나리오 재현 — (a) 연속 크래시 6회 → 정확히 2회째에 탭 재생성
1회 호출, 6회째에 안전 중단, frontier 완전 보존(64개 그대로)·visited 불변·failed 0건
(오염 없음). (b) badSum 3회 → 정상적으로 `failed`에 3개 기록되고 frontier에서 제거
(64→61) — 두 경로가 정확히 분리됨을 확인. `tests/run_all.py --fast` 63/0 통과.
