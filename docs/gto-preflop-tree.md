...
5. **큐 스팟 수집 = 이웃 동시 수집**: 큐(실전 미수집 스팟)를 수집할 때 그 노드만 찍지 말고,
   **경로상 조상 노드(거쳐온 프리픽스) + 빈도>ε인 자손 노드(프리플랍 연속)** 를 함께 수집.
   이미 그 근처까지 navigate 했으므로 재방문(=일일 한도 1스팟 추가 소모)을 아낀다. 즉 큐
   수집도 체계적 수집과 같은 "국소 트리 확장" 메커니즘 — 뿌리만 다르다(체계=빈 시퀀스, 큐=실전 스팟).
...
- **④ 수집 엔진 = 데이터 기반 트리 워커**(위 "데이터 기반 트리 수집" 참조): 노드의 제공
  액션·실측 사이즈·합산 빈도를 화면에서 추출 → ε 초과 액션만 자식으로 확장하며 BFS(얕은
  노드=실전 빈발 우선). 규모 사전열거 없이 큐/일일 한도까지 걷는다. Chrome 수동 수집은
  토큰상 수십 개까지만 현실적 → Playwright 자동화가 사실상 필수.
