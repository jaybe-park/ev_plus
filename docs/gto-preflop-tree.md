...
### 미결 결정 (구현 착수 시 확정)
- **D1**: `canonical_raise_size`/테이블을 런타임 스냅 폴백으로 남길지 완전 제거할지.
  권장: 노드 키 생성에서 제거. 런타임 스냅은 수집 형제 노드에서 사이즈를 읽고, 없으면
  하드코딩 폴백 대신 **큐로 보냄**(추측 금지 원칙).
- **D2**: 레거시 11노드 재키잉을 v13 마이그레이션(알려진 실측 사이즈로)로 할지, 데이터 기반
  워커로 재수집할지. 권장: **워커로 재수집**(얕은 코어라 저렴 + 워커 검증도 됨). 그때까지
  캐노니컬 키는 임시.
- **D3**: 런타임 스냅 허용 오차/포기 임계값(너무 멀면 큐로) 정의.
