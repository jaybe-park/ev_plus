...
- [ ] **프리플랍 전체 트리 커버리지 — 방향성만 확정 (아직 설계/실행 전)** (2026-07-12)
  ...
  **관련 기존 항목**: "포스트플랍 베팅 레인지 반영"(B-2)과 데이터 소비처가
  ...
  - [ ] **② 시퀀스 키 스키마 + 마이그레이션** (sonnet 에이전트, 순수 코드, ① 이후)
    - `docs/gto-preflop-tree.md` "스키마 설계" 안 A/B 중 택1 구현. 노드 키 =
      정규화된 `preflop_actions` 문자열. `hero_position`/`num_active`/`raise_size`는
      시퀀스에서 파생 컬럼으로. 스키마 버전 증가 + 마이그레이션.
    - 기존 11~35개 (position,vs_position,range_type) 행 → 시퀀스 문자열로
      결정론적 변환(마이그레이션 스크립트, 검증 포함). 데이터 손실 0.
    - 조회 시 **사이징 스냅**: 실전 레이즈를 그 노드의 캐노니컬 사이즈로 반올림
      매핑하는 규칙 구현(가장 가까운 캐노니컬, 없으면 미수집 큐로).
    - advisor/loader/save 엔드포인트/봇을 시퀀스 키 조회로 전환.
      `/gto/preflop/save`, `gto/url_generator.py`도 시퀀스 키 대응.
    - 검증: 마이그레이션 후 기존 35개 스팟이 시퀀스 키로 동일 데이터 반환 +
      `audit_gto_preflop.py` 통과 + `run_all.py --full` 통과. 통과 시에만 커밋.
