...
- [ ] **프리플랍 전체 트리 커버리지 — 방향성만 확정 (아직 설계/실행 전)** (2026-07-12)
  ...
  **관련 기존 항목**: "포스트플랍 베팅 레인지 반영"(B-2)과 데이터 소비처가
  ...
  - [ ] **① 구조화된 프리플랍 액션 히스토리** (sonnet 에이전트, 순수 코드)
    - `core/game.py`: 프리플랍 자발적 액션(블라인드 제외)을 `[{position, action:
      fold|call|raise|allin|check, amount_bb}]` 구조로 얻는 헬퍼 추가
      (`preflop_action_seq()` 또는 `event_log`의 "action" 이벤트에 position+street
      포함). position은 `get_positions()`(헤즈업 "BTN/SB" 포함) 사용.
    - `_get_game_state()`에 구조화 시퀀스 노출(예: `preflop_seq`).
    - `gto/advisor.py`: 한글 문자열 파싱(`_count_preflop_raises`/
      `_find_raisers_in_log`)을 구조화 시퀀스 기반으로 교체 → 캐노니컬
      `preflop_actions` 문자열 생성. **기존 35개 enum 조회 경로/봇 행동/모든
      테스트가 깨지지 않아야 함**(어댑터 유지 또는 ②와 함께 전환).
    - 검증: `python3 tests/run_all.py --full` 통과 + 유닛테스트 추가(스퀴즈
      시퀀스가 구조적으로 올바르게 파싱되는지: "UTG오픈-HJ3벳-CO콜-BTN결정"에서
      CO의 콜이 시퀀스에 포함되는지 assert). 통과 시에만 커밋(푸시 금지).
