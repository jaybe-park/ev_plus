...
- [ ] 🔴 **프리플랍 GTO 콜 빈도 전량 유실 — 파서 수정 + 전량 재수집** (긴급, 2026-07-10 확인)

  **증상** (사용자 플레이 체감 + DB 검증으로 확정):
  - 전체 8,619핸드 데이터에서 `freq_call > 0`인 행이 **0개** — vs_open/vs_3bet에
    콜 레인지가 통째로 없음 (RFI는 콜 없는 게 정상이지만 나머지는 명백한 버그)
  - 빈도 합이 1이 안 되는 행 다수 (예: BB vs BTN open의 76s = fold 0 + raise
    0.59%만 존재, 99.4% 증발) — 수집 파서가 call 색상(#22c55e) 세그먼트를 유실
  - **연쇄 피해**: `sample_action()`이 남은 빈도로 폴스루 → 콜해야 할 핸드가
    사실상 100% 레이즈로 변환 → "봇이 올인 많고 과도하게 어그레시브" 체감의 직접 원인.
    hard 봇 `get_call_range`도 항상 빈 값 → 콜러 레인지 반영 무력화.
    플레이 평가의 프리플랍 등급도 오염 데이터 기준

  **실행 계획**
  1. [파서 검증 — 수집 재개 시] CSS background-size 파싱에서 call 색상 처리 확인/수정.
     **저장 시 핸드별 빈도 합 ≈ 1.0(±0.02) 검증 필수** — 아니면 저장 거부하고 경고
     (이번 버그의 재발 방지 핵심. 합 0.6%짜리를 그냥 저장한 게 화근)
  2. [방어 로직 — sonnet 에이전트, 즉시 가능] `gto/loader.py` `sample_action()`이
     빈도 합 < 0.9인 데이터를 만나면 잔여 확률을 fold로 처리하도록 수정
     (재수집 전까지 봇 어그레시브 왜곡 완화) + 로더에서 합 비정상 스팟 경고 로그
  3. [전량 재수집 ~50스팟] Playwright 자동화(아래 항목)의 1차 실전 투입으로 수행.
     재수집 후 콜 빈도 존재 + 합 검증 통과 확인 → 아레나 A/B로 봇 어그레시브 완화 확인
