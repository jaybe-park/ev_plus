...
- [ ] **프리플랍 전체 트리 커버리지 — 방향성만 확정 (아직 설계/실행 전)** (2026-07-12)
  ...
  **관련 기존 항목**: "포스트플랍 베팅 레인지 반영"(B-2)과 데이터 소비처가
  ...
  - [ ] **④ 데이터 기반 트리 워커 + 수집** (별도 세션, GTO Wizard 필요 — 인터넷+크롬 디버그)
    - 핵심 신규 부품: **노드 추출기** — 한 노드에서 제공 액션 + 각 실측 사이즈 +
      레인지 합산 빈도를 화면에서 뽑음. **빈도 > ε(0.05%) 액션만 자식 확장.**
    - 워커 우선순위: **1순위=GTO 트리 체계 수집(빈도/도달확률 가중 best-first — 자주 나오는
      라인부터), 2순위=큐(트리 밖 스팟, 조상+빈도>ε 자손 동시 수집).** 저장 키는 실측 사이즈.
      규모 사전열거 없이 트리 소진/일일 한도까지.
    - Chrome 수동은 토큰상 수십 개까지만 현실적 → Playwright 자동화 사실상 필수
      (TODO "GTO 수집 Playwright 완전 자동화"와 통합/선행).
