...
- [ ] **플레이 평가 기능** (단계별)
  - 1단계: 프리플랍 GTO 빈도 기반 — 핸드 종료 시 내 액션별 평가 표시 (세션 메모리)
  - 2단계: GTO Wizard 수집 시 EV값도 추출 → EV 손실 계산
  - 3단계: DB 저장 → 장기 트래킹 (지난 100핸드 성적)
  - 포스트플랍: equity 기반 근사 평가 (equity_cache 활용 가능)
