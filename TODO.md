...
- [ ] **프리플랍 equity 우선 계산 옵션** (memo 요청)
  - 워커에 `--preflop-first` 플래그: 프리플랍 845스팟 샘플링을 exact보다 우선
  - 구현방안 확정: MC 누적 유지 + vs1은 목표 1000만 샘플로 상향 (±0.015%)
    - 전수조사는 스팟당 21억 조합이라 불가, 공개 정확 테이블과 대조 검증 추가
