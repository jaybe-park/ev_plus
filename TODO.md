...
- [ ] **DB 인프라 정비 (SQLite 유지 + 파일 분리/보존/백업)** — 기획 확정 (2026-07)
  ...
  **⚠️ 트리거 발동 + 원인 재확인 필요 (2026-07-17)**: `poker.db` 실측 **15GB**로
  트리거(5GB) 이미 초과. 행 수 직접 조회 결과, 아래 "작업 2"가 "용량 성장의 실제
  주범"으로 지목한 `players_state`(플레이 기록)는 **이번에 다시 보니 주범이 아닐
  가능성이 높음** — 실측: `equity_cache` 1억 3,977만 행 vs `games`+`preflop_actions`+
  `postflop_actions` 합계 약 314만 행(50배 차이). 대략 계산으로 15GB 중 10GB+가
  `equity_cache`, 플레이 기록은 ~1.5GB 추정(그라인드/워커를 이 기획 수립 이후
  훨씬 많이 돌려서 equity_cache가 그 사이 폭증한 것으로 보임 — 확정 아니고
  정확한 바이트 단위 측정은 아직 안 함, `dbstat` 가상 테이블 미컴파일이라 이번엔
  행 수 기반 추정만 함).
  - **다음에 이것부터 할 것**: (a) equity_cache의 실제 디스크 점유를 정확히
    측정(예: `dbstat` 컴파일된 sqlite3로 재확인, 또는 테이블별 VACUUM INTO 후
    파일 크기 비교) (b) equity_cache는 **재계산 가능한 캐시**(원본 데이터 아님)라
    플레이 기록보다 삭제/축소 리스크가 낮음 — 정리 전략(오래된 MC 근사치 정리,
    필요 없는 num_opponents 조합 정리 등)을 먼저 설계 (c) 그 결과에 따라 "작업 1
    (파일 분리)"을 equity.db 중심으로 먼저 할지, 별도 equity_cache 전용 정리
    작업을 새로 기획할지 결정. **아래 "작업 2"(플레이 기록 아카이브)는 예상보다
    효과가 작을 수 있으니(1.5GB 추정) 우선순위를 낮출 것.**
...
- [~] **GTO 수집 Playwright 완전 자동화** — ④ 데이터 기반 트리 워커로 **대체/흡수됨**
  (2026-07-17). 아래 레거시 열거 방식 설계 대신 `scripts/collect_gto_tree.py`(데이터 기반,
  실측 사이즈 키, 빈도>ε 분기, 도달확률 best-first)가 실제 구현체다. CDP 연결/한도 감지/
  --dry-run/--limit/스팟단위 저장 등 아래 핵심 설계는 그 드라이버에 반영됨. 아래는 기록 보존.
  ("프리플랍 전체 트리 커버리지 → ④" 항목 참조.)
...
- [ ] **프리플랍 전체 트리 커버리지 — 방향성만 확정 (아직 설계/실행 전)** (2026-07-12)

  **목표**: 프리플랍 GTO를 "RFI/vs_open/vs_3bet(오프너 본인만) 35개 조합"이 아니라
  **모든 도달 가능한 프리플랍 의사결정 노드**로 확장. 동기: 정확한 상대 레인지가
  있어야 포스트플랍 equity(`ai/equity.py`의 `ranged_equity`/`RangeSampler`,
  `ai/bot.py`의 `_opponent_ranges`)가 정확해짐 — 지금은 스퀴즈/멀티웨이/4벳+/
  오프너 아닌 제3자 반응 상황에서 GTO 데이터가 아예 없어 equity 휴리스틱으로만
  판단(질문 계기: "UTG 오픈-HJ 3벳-CO 콜" 상황에서 BTN이 아무 GTO 데이터도
  못 받는 걸 확인, `gto/advisor.py`의 `my_position != opener_pos` 가드에서 스킵됨).

  **핵심 통찰 — 포스트플랍과 다른 점**: 포스트플랍이 "전부 수집 불가"인 이유는
  보드 카드 조합 폭발(플랍만 130만+) 때문. 프리플랍은 카드가 없고 순수 액션
  시퀀스뿐이라 트리가 **유한하고 열거 가능** — `docs/gto-postflop.md`처럼 텍스처
  클래스 근사가 아니라 진짜 "전부"를 목표로 잡아도 되는 문제.

  **스키마 방향**: 지금의 `(position, vs_position, range_type)` enum 3종 키를
  **실제 액션 시퀀스 문자열**(GTO Wizard의 `preflop_actions` 파라미터 포맷,
  예: `"F-F-F-R2.5-F-R13.5"`) 자체를 키로 저장하는 방식으로 교체해야 임의의
  노드(스퀴즈든 멀티웨이든)를 일반적으로 담을 수 있음. 기존 35개 조합은
  포지션 순서/range_type 의미로부터 동등한 시퀀스 문자열을 결정론적으로 역산
  가능 — **마이그레이션 가능, 버릴 데이터 아님**.

  **⚠️ 선행 조건 (설계 착수 전 반드시 먼저 처리 — 코드 조사로 확인됨, 2026-07-12)**:
  실시간 게임 진행 중 advisor가 쓸 수 있는 **구조화된 액션 히스토리가 현재
  코드베이스 어디에도 없음**. 상세:
  - `core/game.py`의 `self.event_log`(구조화 이벤트, `{player, action, amount}`)는
    **position 필드가 없고, 게다가 완전히 미사용(dead code)** — `session.py`/
    `recorder.py`/`ai/bot.py` 어디서도 안 읽음
  - 지금 실제로 동작하는 유일한 경로는 `server/session.py`의
    `self.action_log: List[str]`(예: `"UTG 레이즈 2.5bb"` 같은 한글 텍스트)이고,
    `gto/advisor.py`의 `_count_preflop_raises`/`_find_raisers_in_log`가 이걸
    **문자열 부분매칭으로 재파싱**해서 raise_count/오프너 목록을 추측함 —
    콜 여부, 정확한 시퀀스 순서, 멀티웨이 참여자 수를 구조적으로 판별 불가
  - DB의 `preflop_actions` 테이블(`db/schema.py`, position/bet_round/amount 등
    완전 구조화)은 **핸드 종료 후 기록용**이라 실시간 조회 불가
  - **선결 작업**: (a) `event_log`에 position 필드를 추가해 실제로 채워 넣고
    session.py가 소비하도록 연결하거나, (b) session.py에 별도의 구조화된
    `structured_action_log`(포지션+액션+사이즈 리스트)를 신설. 이거 없이는
    시퀀스 키 기반 조회 자체가 불가능하므로 스키마 재설계보다 **먼저** 해야 함

  **⚠️ 사이징 그래뉼래리티 문제**: 실전 플레이(사람/봇)의 베팅 사이즈는 연속값인데,
  GTO Wizard의 솔브 트리는 노드당 정해진 몇 개의 이산 사이즈 옵션만 제공함.
  실시간 조회 시 "가장 가까운 캐노니컬 사이즈로 스냅"하는 전략이 필요 —
  설계 단계에서 반드시 다뤄야 할 문제, 나중에 발견하면 재작업 커짐

  **트리 규모는 추측하지 말 것**: 실제로 GTO Wizard가 노드당 몇 개의 사이즈
  옵션을 제공하는지, 리레이즈가 몇 단계까지 갈리는지(100bb 기준 보통 4~5레이즈
  선에서 올인 강제되지만 실측 필요) 설계 착수 시 GTO Wizard를 직접 몇 개 열어
  실측한 뒤 전체 노드 수를 추정할 것 — 지금 미리 숫자를 정하지 않음

  **수집 엔진 의존성**: 노드 수가 수백~수천 규모가 될 가능성이 높아 지금처럼
  Chrome MCP 수동 수집(토큰 소모)으로는 규모가 안 맞음. 위 "GTO 수집 Playwright
  완전 자동화" 항목이 선행되거나 이 작업이 그 자동화의 실사용 사례가 되어야 함

  **검증 게이트 — 무작정 전체 크롤링 금지**: 스퀴즈/멀티웨이 등 소수 스팟만
  파일럿 수집 → 아레나 bb/100으로 실제 개선 확인(포스트플랍 GTO 추상화 계획과
  같은 원칙, `docs/gto-postflop.md` 참고) → 개선 확인되면 본격 확장. 대부분의
  실전 볼륨(싱글레이즈/싱글3벳)은 이미 지금 35개로 커버되므로, 이 작업의 이득은
  롱테일(스퀴즈/멀티웨이/4벳+) 스팟에 집중됨 — 급하지 않은 장기 과제로 취급

  **관련 기존 항목**: "포스트플랍 베팅 레인지 반영"(B-2)과 데이터 소비처가
  겹침(`_opponent_ranges` 확장) — 함께 검토

  ---

  ### 실행 계획 (2026-07-13 확정, 상세 설계: `docs/gto-preflop-tree.md`)

  단계 순서: ① 구조화 액션 히스토리 → ② 시퀀스 키 스키마 → ③ 트리 정찰(완료) →
  ④ Playwright 자동화 → ⑤ 파일럿 수집+검증 → ⑥ 본격 확장.
  ①② = 순수 코드(인터넷 불필요, 밤새 자율 가능). ③ = 완료. ④⑤⑥ = GTO Wizard
  (인터넷+크롬 디버그 실행) 필요 → 별도 세션.
  ...
  - [x] **③ 트리 규모 정찰** (2026-07-13, Browser 페인으로 실측)
    ...
    - 상세: `docs/gto-preflop-tree.md` "GTO Wizard 트리 구조 실측".
  ...
  - [x] **① 구조화된 프리플랍 액션 히스토리** (완료 2026-07-15, 순수 코드)
    - **구현 방식(택1: event_log 이벤트 강화)**: `core/game.py`의 `apply_action`이
      ...
      (amount_bb = to_amount/big_blind)를 반환. 블라인드는 "blinds" 이벤트라
      "action" 필터에서 자연히 제외(GTO Wizard preflop_actions 포맷과 동일).
      event_log는 미사용(dead code)이었고 외부 소비자 0 → 필드 추가는 하위 호환.
    - `_get_game_state()`에 `preflop_seq` 노출 → `server/session.py`의 advisor
      호출 경로(`_get_gto_hint`/`_grade_human_action`)가 `game._get_game_state()`를
      쓰므로 자동 전달됨(session.py 코드 변경 불필요). UI용 한글 `action_log`는 유지.
    - `gto/advisor.py`: `_count_preflop_raises`/`_find_raisers_in_log`(한글 파싱)을
      `_count_raises`/`_raisers`(preflop_seq 기반)로 교체. 기존 quirk 보존을 위해
      raise 카운트는 "raise"만(allin 제외 — 구 한글 파서가 "레이즈"만 셌던 것과 동일).
      RFI/vs_open/vs_3bet 라우팅, 헤즈업 BTN/SB→SB 매핑, 모델-밖 가드는 그대로.
      vs_open 오프너는 seq 첫 레이저, 레이즈 없이 current_bet>BB인 올인 엣지만
      기존 `find_opener_position`으로 폴백. `canonical_preflop_actions()` 헬퍼 추가
      (F/X/C/R{bb} 포맷, ② 재사용용).
    - **검증**: `run_all.py --full` 전체 통과(poker_full 57 + equity 39 + grader 31).
      유닛테스트 3종 추가(test_poker_full 6-10 스퀴즈 콜 구조화 확인/6-11 헤즈업
      BTN/SB 라벨 유지/6-12 vs_open 시퀀스 라우팅). 실 DB 스팟체크로 open/vs_open/
      vs_3bet(UTG vs HJ) 동일 데이터 반환 확인. (BTN vs BB 3bet은 DB에 vs_position이
      'BB'로 저장돼 로더의 'opener/three_bettor' 키와 불일치 → 리팩터 무관 기존
      데이터 문제로 old/new 모두 None, 동작 동일.)
  ...
  - [x] **② 시퀀스 키 스키마 + 마이그레이션** (완료 2026-07-15, 순수 코드)
    - **택한 스키마 = 안 A**(저위험, additive): `gto_preflop_situations`에 `action_seq`
      ...
      `(position,vs_position,range_type)` enum 키/UNIQUE는 그대로 유지(레거시/파생,
      nullable). 노드 키 유니크는 별도 부분 유니크 인덱스 `idx_gto_pre_seq`. `SCHEMA_VERSION`
      11→12. 안 B(신규 nodes 테이블) 반려: FK 재배선/조회부 이중화 표면이 넓어 non-breaking
      원칙에 안 맞음.
    - **스냅 규칙 = 레이즈 깊이 기준**(사이즈-근접 아님): `url_generator.canonical_raise_size`
      단일 소스(`CANONICAL_OPEN_SIZE` SB 3.5, 그 외 오픈 2.5 / `CANONICAL_RAISE_BY_DEPTH`
      3벳 8·4벳 17.5·5벳 35). 금액이 아니라 **레이즈 순번**으로 스냅 → 13.5bb 3벳이
      4벳(17.5)으로 오분류되는 함정 회피. 저장(백필/save)·런타임(advisor) 동일 함수 사용
      = 키 생성 일원화. 노드 키 생성: `situation_to_node_key`(파생) ↔ `canonical_node_key`(런타임).
    - **마이그레이션 결과**: 11 situations / 1778 hands 보존(손실 0). action_seq 11/11 백필
      전부 distinct·NULL 0. vs_3bet `vs_position` 반쪽 포맷 `"BB"`→`"BTN/BB"` 정규화
      (인계된 불일치 해소 → BTN vs BB 3bet이 enum 경로로도 정상 조회됨). 콜러블 마이그레이션
      스텝 지원을 위해 `db/connection.py`가 SQL 문자열/콜러블 모두 실행하도록 확장.
      실행 전 `poker.db.pre-seq-migration.bak` 백업(scratch, 커밋 대상 아님).
    - **조회 경로**: `loader.get_range_by_seq(node_key)` 추가(`_cache_by_seq`, enum 캐시와
      동일 객체 공유). `advisor.get_recommendation`은 enum 경로 우선(전 동작·quirk·큐 보존)
      + enum None일 때 시퀀스 키 폴백(additive, 현재 롱테일 데이터 없어 항상 None → 동작 불변).
      `/gto/preflop/save`가 `action_seq` 수신·파생·저장, vs_3bet 정규화. `url_generator`에
      `url_from_node_key` 추가 + `vs_3bet_url` 선행 폴드 누락 버그 수정(④ 재사용).
    - **검증(통과)**: `run_all.py --full` 전체 통과(poker_full 60 + equity 39 + grader 31).
      유닛테스트 3종 추가(6-13 enum키=시퀀스키 동일 레인지 / 6-14 런타임 사이즈 스냅→노드
      매핑 / 6-15 마이그레이션 vs_3bet 포맷 정규화). `audit_gto_preflop.py` 통과. 실 DB에서
      11개 전부 enum↔seq 동일 객체 반환 확인. 상세: `docs/gto-preflop-tree.md` "② 구현 결정".
    - **사용자 검토 필요**: ①3벳+ 깊이를 단일 캐노니컬(8/17.5/35)로 뭉갠 근사가 충분한지
      (④/⑤ 파일럿에서 판단) ②allin 노드 키 표현(현재 깊이 스냅, 실측 allin 노드 수집 시 재검토)
      ③num_active 정의 충분성. → 다음은 ④ Playwright 자동화(GTO Wizard 필요, 별도 세션).

  - [x] **②' 데이터 기반 키/스냅 전환** (2026-07-16 확정 → 2026-07-17 구현 완료, 순수 코드)
    상세 설계·코드 영향 분석·서브에이전트 지시: `docs/gto-preflop-tree.md`
    "데이터 기반 트리 수집 방식" + "②' 구현 완료" 섹션.
    - **완료 요약**: D1(하드코딩 캐노니컬 테이블을 노드 키 생성에서 제거 — 레거시
      `situation_to_node_key` 전용으로 격리) + D3(런타임 `canonical_node_key`를
      **트리-인지 스냅**으로 재작성: `loader.get_children_by_prefix`로 수집된 형제
      노드의 실측 사이즈에 스냅, 미수집 브랜치는 None→실측 키로 큐 등록) 구현.
      저장 키는 실측 사이즈 verbatim(save의 action_seq 우선 — 기존 동작 유지). D2(레거시
      11노드 재수집)는 ④ 워커 몫으로 남김(v13 없음). 스키마 컬럼 3종 불변.
    - **검증(통과)**: `run_all.py --full` 전체 통과(poker_full 63 + equity 54 + grader 31).
      유닛테스트 3종 추가(6-16 실측 사이즈 형제 스냅 / 6-17 미수집 브랜치 None+큐 등록 /
      6-18 형제 2개 bb 최소거리 스냅). `audit_gto_preflop.py` 통과. 실 DB 스팟체크:
      라이브 3벳 11bb가 수집 형제 R8로 스냅돼 레거시 11노드 정상 조회(회귀 없음).
    - **배경**: ②는 노드 키를 깊이-캐노니컬 사이즈(3벳=항상 R8)로 저장했으나,
      데이터 기반 수집은 **실측 사이즈 키**(BB 3벳=R13.5) + **런타임 트리-인지 스냅**으로
      전환해야 함(사용자 확정). ②의 `url_from_node_key`가 3벳+ URL을 틀리게 만들던
      결함도 실측 키로 해소됨.
    - **바뀌는 것**(재작업, 문서 표 참조): `url_generator`의 `CANONICAL_*`/
      `canonical_raise_size`/`situation_to_node_key`(노드 키 생성에서 제거·축소),
      `advisor.canonical_node_key`(깊이 스냅 순수함수 → **트리-인지 나머지 스냅**, 핵심),
      `backfill_v12` 11노드 재키잉(v13 or 워커 재수집).
    - **유지되는 것**(건드리지 말 것): ① `preflop_action_seq`, 스키마 컬럼 3종,
      큐 테이블, `save`의 action_seq 수신, `canonical_preflop_actions`,
      `loader.get_range_by_seq` 정확 매칭(스냅은 advisor가 하고 정확 키를 넘김).
    - **확정 결정(2026-07-16, 문서 "확정 결정 D1~D3" 참조)**: D1=하드코딩 캐노니컬 테이블을
      노드 키 생성에서 제거(스냅은 수집 형제 기준, 없으면 큐). D2=레거시 11노드는 v13 없이
      ④ 워커로 재수집(실측 키로 덮어씀). D3=라이브 레이즈를 프리픽스의 수집된 레이즈-형제로
      스냅(노드당 1개라 보통 유일, 2개+면 bb 최소거리), 미수집 브랜치는 큐+휴리스틱.
    - 검증: 기존 enum 경로·봇·테스트 유지(가능하면 병렬 공존), `run_all.py --full` 통과 게이트.

  - [x] **④ 데이터 기반 트리 워커 + 수집 — 드라이버 구현 완료** (2026-07-17, GTO Wizard 필요)
    - 순수 로직: `scripts/gto_tree_worker.py`(집계/분기ε/토큰/도달확률 큐) — 기존.
    - **드라이버 신규: `scripts/collect_gto_tree.py`** — 위 순수 로직에 Playwright(CDP)
      ...
      - 중단-재개: `gto_tree_checkpoint.json`(visited/frontier/failed) + DB action_seq로
        재개. 체크포인트 없으면 DB 기수집 트리에서 프론티어 재구성(seed).
      - 일일 한도 감지: **실측 확정**(2026-07-17) — 상단 `X/100` 카운터가 유일한 권위
        신호(문구는 항상 떠있는 툴팁이라 단독 판단 불가). `--safety-margin`(기본 5) 이하로
        떨어지면 안전 종료. `--limit`(기본 90)은 이보다 커도 안전마진에서 자동 정지.
    ...
    - **실전 라이브 실행 완료(2026-07-17)**: 사용자가 크롬 디버그+로그인 후 여러 차례
      ...
      4. (세 번째 실행, `--limit 200`) 100번+ 반복 navigate로 **크롬 렌더러 크래시**
         발생(`chrome://crashes`로 확인 — 서버 차단 아님, 브라우저 리소스 문제) →
         이후 20개 연속 실패, 크래시로 실패한 노드가 badSum과 동일 취급돼 `failed`/
         `visited`에 영구 기록될 뻔함(84개, 세션 중 DB 기준 체크포인트 재구성으로
         긴급 복구). **코드 수정**: `is_env_failure()`로 환경오류(크래시/타임아웃/
         저장실패)와 진짜 데이터이상(badSum) 분리 → 환경오류는 frontier로 되돌려
         자동 재시도, 연속 2회면 탭 자동 재생성(로그인 세션 유지), 연속 6회면 안전
         중단(서킷브레이커), 25개마다 예방적 탭 재생성. 몽키패치로 두 경로(크래시/
         badSum) 검증 — frontier 완전 보존, failed 오염 없음. 상세: `docs/gto-preflop-tree.md`
         "세 번째 라이브 실행" 섹션.
    - 사용법 상세(사전 조건 체크리스트 포함): `docs/gto-preflop-tree.md`
      "사용자 실행 방법" 섹션.
  - [x] **수집 현황 리포트** (2026-07-17) — `scripts/gto_tree_report.py` 신규.
    ...
    collected(초록)/frontier(노랑)/failed(빨강)/미방문 조상(회색 점선) 3+1분류로만 표시.
    사용자가 직접 재실행 가능: `python3 scripts/gto_tree_report.py`.
  - [ ] **⑤ 파일럿 + 검증**: 스퀴즈/멀티웨이 소수 노드 수집 → 아레나 bb/100으로
      "equity 휴리스틱 대비 개선" 확인. 개선 없으면 롤백. (D1의 "3벳+ 단일 캐노니컬
      근사 충분성"도 여기서 판정.)
  - [ ] **⑥ 본격 확장**: 파일럿 개선 확인 시 헤즈업(≈220) → ≤3-way(≈2,665)로 확장.
      "전부(≈9.2만)"는 가성비상 목표 아님.

  - **문서 업데이트**: 각 단계 완료 시 `docs/gto-preflop-tree.md`,
    `docs/db-schema.md`(② 스키마), `docs/architecture.md`(데이터 흐름), 이 항목 갱신.
