...
- [ ] **에퀴티 계산 2차 고속화** — 기획 확정 (2026-07). 곱연산 순서로 진행.
  ...
  - [ ] **작업 1 — PyPy 실행 시험** (공수 ~0, 먼저 확인)
    - `pypy3` 설치 후 `pypy3 scripts/equity_worker.py --minutes 1` 처리량을
      `python3` 대비 측정 (JIT라 코드 무변경 5~20배 기대)
    - sqlite3/pyarrow 등 C확장 의존성이 pypy에서 되는지 확인 (안 되면 순수 파이썬 경로만)
    - 결과가 좋으면: docs/ai-bot.md에 "워커는 pypy3로 실행" 명시 + grind/워커 실행 안내 갱신
    - 담당: 메인이 직접 (설치+측정만, 코딩 거의 없음). 커밋은 문서 갱신 시만

  - [ ] **작업 2 — 보드 중심 리버 계산 재구조화** (sonnet 에이전트, 반나절)
    - 아이디어: 같은 보드를 공유하는 스팟들이 상대 990조합 랭크 판정을 매번 반복.
      보드 B당 가능한 홀 C(47,2)=1081개 랭크를 **한 번만** 계산해 정렬 테이블화 →
      각 홀 equity = 이진탐색으로 "나보다 낮은 랭크 수" 카운트 (블로커 89조합만 보정)
    - 효과: 보드 하나로 그 보드의 모든 홀 equity가 공짜. 플랍 스윕(보드 반복)에서 극대화
    - 구현: `ai/equity.py`에 `board_equity_table(board)` + 스윕/전수조사 경로가 이를 사용.
      기존 exact_counts_river와 **동일 결과 보장**(E-2 등가성 테스트 확장)
    - (eval7 C 평가기 도입은 대안 — pypy 결과 보고 판단. 보드 중심과 곱연산 가능)
    - 검증: 랜덤 보드 100개에서 보드중심 vs 기존 전수조사 완전 일치, 처리량 before/after
    - 커밋: ai/equity.py + 테스트 + docs/ai-bot.md, 다중 -m 메시지, push
