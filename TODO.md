  ...
  **관련 기존 항목**: "포스트플랍 베팅 레인지 반영"(B-2)과 데이터 소비처가
  ...
  - [x] **④ 데이터 기반 트리 워커 + 수집 — 드라이버 구현 완료** (2026-07-17, GTO Wizard 필요)
    ...
    - **드라이버 신규: `scripts/collect_gto_tree.py`** — 위 순수 로직에 Playwright(CDP)
      ...
      - 일일 한도 감지: 페이지 텍스트/렌더실패 휴리스틱(⚠️ **추측 기반 — 실제 실행하며
        신호 관찰해 다듬어야 함**). `--limit`(기본 90) 안전마진.
      - CLI: `--limit/--dry-run/--cdp-url/--server/--checkpoint/--epsilon/--nav-timeout`.
      - CDP 연결 실패(크롬 미실행) 시 크래시 아니라 안내 출력 후 exit 0 (검증 완료).
    - **남은 것 = 사용자가 크롬 디버그 모드 실행 후 실제 수집 실행**(아래):
      1. `/Applications/Google Chrome.app/Contents/MacOS/Google Chrome --remote-debugging-port=9222 --user-data-dir="$HOME/chrome-gto-debug"`
      2. 그 창에서 GTO Wizard 로그인
      3. 서버 실행(`./start.sh`) 후 `python3 scripts/collect_gto_tree.py --dry-run`으로
         파서 눈검증 → `--limit N`으로 실전 수집. D2(레거시 11노드)는 워커가 실측 키로
         자연히 덮어씀.
    - 사용법 상세: `docs/gto-preflop-tree.md` "④ 드라이버 사용법" 섹션.
