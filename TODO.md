...
- [ ] 🔴 **HJ RFI 데이터 오염 확인 + 전체 프리플랍 GTO 스팟 전수 검사** (memo 피드백, 2026-07-12)
  ...
  - 실행 계획 (sonnet 에이전트 1개)
    1. HJ RFI 스팟을 GTO Wizard에서 재확인(현재 레이어 기반 파서로 재추출,
       badSum 검증) → 실제로 HJ가 저 시점에 폴드 100%였는지, 아니면 수집/저장
       과정의 버그였는지 원인 확정
    2. **전체 프리플랍 스팟 전수 검사 스크립트** 작성(`scripts/` 또는 임시
       점검용): 저장된 모든 `gto_preflop_situations`에 대해
       - 핸드별 빈도합이 [0.9,1.1] 안인지(로더 방어와 별개로 원본 DB 자체 검증)
       - 같은 range_type 내에서 포지션 간 오픈 비율이 상식적인 순서인지
         (RFI: UTG < HJ < CO < BTN < SB 넓어지는 방향, 역전되면 의심)
       - 특정 액션이 부자연스럽게 100%/0%로 쏠린 스팟이 있는지(HJ RFI처럼)
       전수 결과를 사람이 읽을 수 있는 리포트로 출력(문제 스팟 목록 + 사유)
    3. 발견된 이상 스팟은 재수집 큐(`gto_missing_spots_preflop`)에 재등록하거나
       직접 재수집, 검증 통과 후 저장(기존 원칙: 검증 실패 시 절대 추측으로
       채우지 않음)
    4. **문서 업데이트**: `docs/gto-data.md`에 전수검사 결과/재수집 이력 반영
  - 완료 기준: 전수 검사 스크립트가 0개 이상 스팟 통과 확인 + HJ RFI 정상화
    + `python3 tests/run_all.py --fast` 통과
