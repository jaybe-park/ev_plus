...
- [ ] 🔴 **프리플랍 GTO 전면 재작업 — 근본 원인 확정 + 사이징 실측화 + 전량 재수집**
  ...
  **배수가 일정하지 않음** — 공식으로 추론 불가, 반드시 실측해서 저장해야 함.
  ...
  1. [스키마 변경 — sonnet 에이전트] `gto_preflop_situations.raise_size`를
  ...
  3. [코드 정리 — 같은 에이전트] `ai/bot.py` `_preflop_raise()`의 하드코딩
     ...
     단계를 진행하는 방식으로 전환.
  4. [로더 방어 — 같은 에이전트] `gto/loader.py`에 로드 시점 빈도합 재검증
