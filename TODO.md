...
- [ ] 🔴 **프리플랍 GTO 빈도 데이터 손상 — 전량 재수집 필요** (긴급, 2026-07-10 확인·수정)

  ⚠️ **1차 진단 정정**: 처음엔 "콜 색상 파싱만 실패"로 단정했으나, 실제 DB 검증 결과
  **콜 하나만의 문제가 아니다** — 어느 색상(액션)이든 유실될 수 있다는 전제로 재조사.

  **증상** (DB 전수 검증, 2026-07-10):
  | range_type | call>0 | allin>0 | 빈도합 비정상 |
  |---|---|---|---|
  | open (RFI) | 0/845 | 0/845 | 74/845 (9%) |
  | vs_open | 0/2535 | 0/2535 | **391/2535 (15%)** |
  | vs_3bet | 0/5239 | 61/5239 | 83/5239 (2%) |

  - **콜은 전 구간(8,619핸드)에서 완전히 0** — 명백한 유실
  - **올인은 vs_3bet엔 정상 존재(3벳팟 잼은 실제로 있음), open/vs_open은 0** —
    100bb 딥스택에서 원래 드문 것일 수도, 유실일 수도 있어 **단정 불가**
  - vs_open의 15%가 빈도 합 붕괴 — 콜 외 다른 세그먼트도 산발적 유실 가능성
  - **중요**: 실제 추출 스크립트는 저장된 파일이 아니라 Chrome MCP로 그때그때
    실행한 1회성 JS였음 — "고칠 코드"가 없고, Playwright 자동화를 **처음부터
    올바르게 설계**해야 함

  **연쇄 피해**: `sample_action()`의 폴스루로 손상된 핸드가 실제와 다른 액션으로
  치환됨(어느 방향인지는 핸드마다 다름) → 봇의 프리플랍 판단 전반 왜곡(체감상
  올인 과다). hard 봇 `get_call_range` 무력화. 플레이 평가 프리플랍 등급도 오염 데이터 기준

  **실행 계획**
  1. [수집기 설계 원칙 — Playwright 자동화 구현 시 반영] 4개 액션 색상
     (fold #3b82f6 / call #22c55e / raise #ef4444 / allin #991b1b)을
     **동등한 방식으로, 특정 액션을 특별 취급하지 않고** 파싱. 핸드별 빈도 합
     ≈ 1.0(±0.02) 검증 후에만 저장 — 실패 시 그 핸드는 거부하고 재시도/경고
     (어떤 색상이 깨지든 잡아내는 게 핵심, "콜만 확인"하면 재발함)
  2. [방어 로직 — sonnet 에이전트, 즉시 가능] `gto/loader.py`에 스팟 로드 시
     핸드별 빈도 합 검증 추가. **합이 비정상이면 특정 액션(예: fold)에 잔여를
     몰아주지 말고, 해당 핸드는 "데이터 없음"으로 처리해 휴리스틱 폴백** —
     어느 색상이 깨졌는지 모르는 상태에서 "부족분=fold"라고 추가로 가정하면
     새로운 오류를 얹는 것과 같음. 손상 스팟은 재수집 대상으로 로그/표시
  3. [전량 재수집 ~50스팟] Playwright 자동화 1차 실전 투입으로 수행.
     재수집 후 4개 액션 전부 합리적 분포 + 빈도합 검증 통과 확인 →
     아레나 A/B로 봇 판단 정상화 확인 (올인 과다 완화 여부 포함)
...
- [ ] **포스트플랍 GTO 추상화** — 설계 확정 (2026-07-10, 상세: docs/gto-postflop.md)
  - 요약: 전부 수집 불가(플랍 텍스처 1,755, 정확 보드 수백만) →
    ...
    ② 실전 우선순위 큐(gto_missing_spots 확장) ③ 미수집 클래스는 equity 폴백(현행)
  ...
  **실행 단계** (순서대로, 각각 sonnet 에이전트)
  ...
  2. [큐 확장] advisor에 플랍 훅 추가: 플랍 데이터 없으면 gto_missing_spots에
     street='flop' + 클래스 키로 기록 (UNIQUE saturate 가능해짐).
...
- [ ] **vs_3bet 나머지 4개 수집** — 아래 "Playwright 완전 자동화" 3단계에서 함께 소진 예정
  ...
  - BTN vs BB 3bet
  - SB vs BB 3bet
  - 참고: 정리 후 vs_3bet 큐에 남은 12건은 위 4개(오프너=BTN/SB 계열)에 대한
    다양한 my_position 대응 스팟들이며, 오탐이 아니라 실제 미수집 갭이다.
